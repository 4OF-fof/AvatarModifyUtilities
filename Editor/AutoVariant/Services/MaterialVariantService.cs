...
namespace AMU.Editor.AutoVariant.Services
{
    public static class MaterialVariantService
    {
        ...
        private static bool ValidateInput(GameObject targetObject, out GameObject parentPrefab)
        {
            ...
            {
                Debug.LogError($"[MaterialVariantService] {LocalizationAPI.GetText("AutoVariant_message_error_avatar_null")}");
                return false;
            }

            if (PrefabUtility.GetPrefabInstanceStatus(targetObject) != PrefabInstanceStatus.Connected)
            {
                Debug.LogError($"[MaterialVariantService] {LocalizationAPI.GetText("AutoVariant_message_error_not_prefab_instance")}");
                return false;
            }
            ...
            if (prefabAsset == null)
            {
                Debug.LogError($"[MaterialVariantService] {LocalizationAPI.GetText("AutoVariant_message_error_prefab_asset_not_found")}");
                return false;
            }
            ...
            {
                Debug.LogWarning($"[MaterialVariantService] {LocalizationAPI.GetText("AutoVariant_message_warning_not_variant")}");
                return false;
            }
        ...
        private static bool ProcessRendererMaterials(Renderer variantRenderer, Renderer parentRenderer, string objectName)
        {
            ...
            {
                Debug.LogWarning($"[MaterialVariantService] {string.Format(LocalizationAPI.GetText("AutoVariant_message_warning_material_count_mismatch"), objectName)}");
                return false;
            }
            ...
            for (int i = 0; i < variantMaterials.Length; i++)
            {
                if (TryOptimizeMaterial(variantMaterials[i], parentMaterials[i], out var optimizedMaterial))
                {
                    ...
                    Debug.Log($"[MaterialVariantService] {string.Format(LocalizationAPI.GetText("AutoVariant_message_info_material_optimized"), objectName, i)}");
                }
                else
                {
        ...
        private static void ApplyPrefabOverride(Renderer renderer, string objectName)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(prefabPath))
                {
                    ...
                    Debug.Log($"[MaterialVariantService] {string.Format(LocalizationAPI.GetText("AutoVariant_message_info_override_applied"), objectName)}");
                }
            }
            catch (Exception e)
            {
                Debug.LogError($"[MaterialVariantService] {string.Format(LocalizationAPI.GetText("AutoVariant_message_error_override_failed"), objectName, e.Message)}");
            }
        }
