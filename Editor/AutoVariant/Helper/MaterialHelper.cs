...
public static class MaterialVariantOptimizer
{
    public static void OptimizeMaterials(GameObject targetObject)
    {
        if (!ValidateInput(targetObject, out var parentPrefab))
            return;

        ProcessMaterialsRecursive(targetObject, parentPrefab);
    }
    ...
    private static bool ValidateInput(GameObject targetObject, out GameObject parentPrefab)
    {
        ...
        {
            Debug.LogError("[MaterialVariantOptimizer] Target object is null");
            return false;
        }

        if (PrefabUtility.GetPrefabInstanceStatus(targetObject) != PrefabInstanceStatus.Connected)
        {
            Debug.LogError("[MaterialVariantOptimizer] Object is not a prefab instance");
            return false;
        }
        ...
        if (prefabAsset == null)
        {
            Debug.LogError("[MaterialVariantOptimizer] Could not find corresponding prefab asset");
            return false;
        }
        ...
        {
            Debug.LogWarning("[MaterialVariantOptimizer] This prefab doesn't seem to be a variant");
            return false;
        }
    ...
    private static bool ProcessRendererMaterials(Renderer variantRenderer, Renderer parentRenderer, string objectName)
    {
        ...
        {
            Debug.LogWarning($"[MaterialVariantOptimizer] Material count mismatch in {objectName}");
            return false;
        }
        ...
        for (int i = 0; i < variantMaterials.Length; i++)
        {
            if (TryOptimizeMaterial(variantMaterials[i], parentMaterials[i], out var optimizedMaterial))
            {
                ...
                Debug.Log($"[MaterialVariantOptimizer] Optimized material {i} in {objectName}");
            }
            else
            {
    ...
    private static void ApplyPrefabOverride(Renderer renderer, string objectName)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(prefabPath))
            {
                ...
                Debug.Log($"[MaterialVariantOptimizer] Applied override to variant prefab for {objectName}");
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError($"[MaterialVariantOptimizer] Error applying override for {objectName}: {e.Message}");
        }
    }
}
